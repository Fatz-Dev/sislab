...
@section('title', 'Manajemen Admin - SISLAB')
...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-ink dark:text-white">Data Admin</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Daftar akun administrator sistem.</p>
        </div>
        <button onclick="toggleModal('modal-add', true)" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-800 transition-colors">
            + Tambah Admin
        </button>
    </div>
    ...
    <div class="bg-white dark:bg-[#171d25] border border-slate-100 dark:border-[#344150] rounded-xl shadow-sm p-6 overflow-hidden">
        <div class="overflow-x-auto">
            <table id="table-admin" class="w-full text-sm text-left text-slate-500 dark:text-slate-400">
...
<div id="modal-add" class="fixed inset-0 z-[99] hidden flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-[#171d25] rounded-xl shadow-lg w-full max-w-md border border-slate-100 dark:border-[#344150] overflow-hidden">
        <div class="flex justify-between items-center p-4 border-b border-slate-100 dark:border-[#344150]">
            <h3 class="text-lg font-semibold text-ink dark:text-white">Tambah Admin</h3>
        ...
        <form action="#" method="POST" class="p-4 space-y-4">
            ...
            <div class="pt-2 flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary/90">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-edit" class="fixed inset-0 z-[99] hidden flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-[#171d25] rounded-xl shadow-lg w-full max-w-md border border-slate-100 dark:border-[#344150] overflow-hidden">
        <div class="flex justify-between items-center p-4 border-b border-slate-100 dark:border-[#344150]">
            <h3 class="text-lg font-semibold text-ink dark:text-white">Edit Admin</h3>
        ...
        <form id="form-edit" action="#" method="POST" class="p-4 space-y-4">
            ...
            <div class="pt-2 flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary/90">Update</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-delete" class="fixed inset-0 z-[99] hidden flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-[#171d25] rounded-xl shadow-lg w-full max-w-sm border border-slate-100 dark:border-[#344150] overflow-hidden text-center p-6">
        ...
        <h3 class="text-lg font-bold text-ink dark:text-white mb-2">Hapus Admin?</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Tindakan ini tidak dapat dibatalkan. Admin yang dihapus akan kehilangan akses ke sistem.</p>
...
@push('scripts')
    ...
    <script>
                        ...
                        render: function(data, type, row) {
                            ...
                            return `
                                <div class="flex gap-2">
                                    ...
                                    <button onclick="openEditModal(${row.id}, '${safeName}', '${row.email}')" class="p-1.5 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400"><i class="bi bi-pencil-square"></i></button>
        ...
        function openEditModal(id, name, email) {
            ...
            // Set form action jika backend sudah siap, misal:
            // document.getElementById('form-edit').action = '/admin/users/admin/' + id;
        ...
        function openDeleteModal(id) {
            // Set form action jika backend sudah siap, misal:
            // document.getElementById('form-delete').action = '/admin/users/admin/' + id;
